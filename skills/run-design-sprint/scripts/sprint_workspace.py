...
def load_method_contract() -> dict[str, Any]:
    data = read_json(REFERENCES_DIR / "method-profiles.json")
    if data.get("schemaVersion") != FIDELITY_SCHEMA_VERSION:
        raise SprintError(
            f"Unsupported method-profile schema: {data.get('schemaVersion')}"
        )
...
def load_state(workspace: Path) -> dict[str, Any]:
    ...
    if version == LEGACY_STATE_SCHEMA_VERSION:
        ...
        raise SprintError(f"Unsupported state schema: {version}")
...
def selection_record(selected_by: str, reason: str) -> dict[str, str]:
    return {
...
def build_fidelity(method_profile: str, execution_mode: str) -> dict[str, Any]:
    contract = load_method_contract()
    steps: dict[str, Any] = {}
    method_key = (
        "bookDefaultMethod"
...
def add_skip_deviation(
    state: dict[str, Any], step_id: str, reason: str, source: str = "explicit-skip"
) -> None:
...
def migrate_legacy_state(state: dict[str, Any]) -> dict[str, Any]:
    ...
    migrated["fidelity"] = build_fidelity(
        migrated["methodProfile"], migrated["executionMode"]
    )
    ...
    for step_id in migrated["skippedSteps"]:
        if step_id in STEP_INDEX:
            ...
            add_skip_deviation(migrated, step_id, reason, "legacy-skip")
...
def command_set_artifact_status(args: argparse.Namespace) -> None:
    ...
    data["updatedAt"] = now
    state = load_state(workspace)
    errors = artifact_data_errors(data, load_artifact_specs())
    errors.extend(evidence_claim_errors(data, state))
    if errors:
...
def workspace_errors(workspace: Path) -> list[str]:
    ...
    for data_path in sorted((workspace / "artifact-data").glob("*.json")):
        ...
        errors.extend(
            f"{data_path.name}: {item}"
            for item in evidence_claim_errors(data, state)
        )
        artifact_id = data.get("id")
